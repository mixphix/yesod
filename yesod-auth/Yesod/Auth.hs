...
type MonadAuthHandler master m =
  ( MonadHandler m
  , YesodAuth master
...
class
  (Yesod master, PathPiece (AuthId master), RenderMessage master FormMessage) =>
  YesodAuth master
  where
  type AuthId master
  ...
  authLayout ::
    (MonadHandler m, HandlerSite m ~ master) => WidgetFor master () -> m Html
  ...
  -- @since 1.2.0
  maybeAuthId ::
    (MonadHandler m, master ~ HandlerSite m) => m (Maybe (AuthId master))
  default maybeAuthId ::
    ( MonadHandler m
    , master ~ HandlerSite m
  ...
  onErrorHtml ::
    (MonadHandler m, HandlerSite m ~ master) => Route master -> Text -> m Html
...
{-# DEPRECATED
  getAuthId
  "Define 'authenticate' instead; 'getAuthId' will be removed in the next major version"
  #-}
