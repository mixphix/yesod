...
verifyR eid verkey hasSetPass = PluginR "email" path
 where
  path = "verify" : eid : verkey : [verifyURLHasSetPassText | hasSetPass]
...
data PasswordForm = PasswordForm
  {_passwordCurrent :: Text, _passwordNew :: Text, _passwordConfirm :: Text}
...
class
  ...
  addUnverifiedWithPass ::
    Email -> VerKey -> SaltedPass -> AuthHandler site (AuthEmailId site)
  ...
  afterPasswordRouteHandler = afterPasswordRoute <$> getYesod
  ...
  --
  -- @since 1.6.4
  emailPreviouslyRegisteredResponse ::
    (MonadAuthHandler site m) => Text -> Maybe (m TypedContent)
 ...
 where
  ...
  dispatch "GET" ["verify", eid, verkey, hasSetPass] =
    case fromPathPiece eid of
      Nothing -> notFound
      Just eid' ->
        getVerifyR eid' verkey (hasSetPass == verifyURLHasSetPassText) >>= sendResponse
...
defaultRegisterHelper ::
  (YesodAuthEmail master) =>
  -- | Allow lookup via username in addition to email
  Bool ->
  -- | Set to `True` for forgot password flow, `False` for new account registration
  Bool ->
  Route Auth ->
...
defaultRegisterHelper allowUsername forgotPassword dest = do
  ...
  case eidentifier of
    ...
    Right identifier -> do
     ...
     where
      sendConfirmationEmail (lid, _, verKey, email) = do
        render <- getUrlRender
        tp <- getRouteToParent
...
-- @since 1.2.6
defaultForgotPasswordHandler ::
  (YesodAuthEmail master) => AuthHandler master Html
...
postForgotPasswordR ::
  (YesodAuthEmail master) => AuthHandler master TypedContent
...
-- @since 1.2.6
defaultSetPasswordHandler ::
  (YesodAuthEmail master) => Bool -> AuthHandler master TypedContent
...
saltPass =
  fmap (decodeUtf8With lenientDecode)
...
saltPass' salt pass =
  salt
    ++ T.unpack
      ( TE.decodeUtf8 $
          B16.encode $
            convert (H.hash (TE.encodeUtf8 $ T.pack $ salt ++ pass) :: H.Digest H.MD5)
      )
