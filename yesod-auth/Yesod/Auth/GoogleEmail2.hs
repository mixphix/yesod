...
module Yesod.Auth.GoogleEmail2
  {-# DEPRECATED
    "Google+ is being shut down, please migrate to Google Sign-in https://pbrisbin.com/posts/googleemail2_deprecation/"
    #-}
 ...
 where
  ...
  dispatch "GET" ["complete"] = do
    ...
    unless (tokenType' == "Bearer") $
      error $
        "Unknown token type: " ++ show tokenType'
...
instance FromJSON PersonURI where
  parseJSON = withObject "PersonURI" $ \o ->
    PersonURI
      <$> o .:? "label"
      <*> o .:? "value"
      <*> o .:? "type"
...
resizePersonImage (PersonImage uri) size =
  PersonImage $ uri <> "?sz=" <> T.pack (show size)
