...
import Network.HTTP.Types (Status (Status), queryToQueryText)
...
parseWaiRequest env session useToken mmaxBodySize =
 ...
 where
  ...
  langs' =
    catMaybes
      [ lookup langKey gets -- Query _LANG
...
randomString len gen =
  decodeUtf8 . fromByteVector <$> V.replicateM len asciiChar
 where
  asciiChar =
    let loop = do
          x <- gen
...
mkFileInfoFile name ct fp =
  FileInfo
    name
    ct
    (sourceFile fp)
    (\dst -> runConduitRes $ sourceFile fp .| sinkFile dst)

mkFileInfoSource ::
  Text -> Text -> ConduitT () ByteString (ResourceT IO) () -> FileInfo
