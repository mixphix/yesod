...
import Data.Conduit (ConduitT, Flush)
...
import Yesod.Core.TypeCache (KeyedTypeMap, TypeMap)
...
data RunHandlerEnv child site = RunHandlerEnv
  ...
  -- Since 1.2.0
  , rheMaxExpires :: !Text
  , rheCatchHandlerExceptions ::
      !(forall a m. (MonadUnliftIO m) => m a -> (SomeException -> m a) -> m a)
  -- ^ @since 1.6.24.0
...
instance (a ~ ()) => IsString (WidgetFor site a) where
  fromString = toWidget . toHtml . T.pack
   where
    toWidget x = tellWidget mempty{gwdBody = Body (const x)}
...
data Script url = Script
  {scriptLocation :: !(Location url), scriptAttributes :: ![(Text, Text)]}
  deriving (Show, Eq)
data Stylesheet url = Stylesheet
  {styleLocation :: !(Location url), styleAttributes :: ![(Text, Text)]}
  deriving (Show, Eq)
...
instance Semigroup (GWData a) where
  GWData a1 a2 a3 a4 a5 a6 a7 a8
    <> GWData b1 b2 b3 b4 b5 b6 b7 b8 =
      GWData
        (mappend a1 b1)
        (mappend a2 b2)
        (mappend a3 b3)
