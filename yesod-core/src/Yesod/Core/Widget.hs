  ...
  toWidget x =
    tell $
      GWData
        ...
        mempty
        mempty
        (Map.singleton Nothing $ unCssBuilder . x)
        mempty
        mempty
  ...
  toWidget x =
    tell $
      GWData
        ...
        mempty
        mempty
        (Map.singleton Nothing $ unCssBuilder . const x)
        mempty
        mempty
...
instance ToWidget site Javascript where
  toWidget x =
    tell $ GWData mempty mempty mempty mempty mempty mempty (Just $ const x) mempty
  ...
  toWidgetMedia media x =
    tell $
      GWData
        ...
        mempty
        mempty
        (Map.singleton (Just media) $ unCssBuilder . x)
        mempty
        mempty
  ...
  toWidgetMedia media x =
    tell $
      GWData
        ...
        mempty
        mempty
        (Map.singleton (Just media) $ unCssBuilder . const x)
        mempty
        mempty
...
setTitle x =
  tell $
    GWData mempty (Last $ Just $ Title x) mempty mempty mempty mempty mempty mempty
...
setDescriptionIdemp description =
  tell $
    GWData
      mempty
      mempty
      (Last $ Just $ Description description)
      mempty
      mempty
      mempty
      mempty
...
addStylesheetRemoteAttrs x y =
  tell $
    GWData
      ...
      mempty
      mempty
      (toUnique $ Stylesheet (Remote x) y)
      mempty
      mempty
      mempty
...
-- | Link to the specified local script.
addScriptAttrs ::
  (MonadWidget m) => Route (HandlerSite m) -> [(Text, Text)] -> m ()
addScriptAttrs x y =
  tell $
    GWData
      mempty
...
addScriptRemoteAttrs x y =
  tell $
    GWData
      mempty
      mempty
      mempty
      (toUnique $ Script (Remote x) y)
      mempty
      mempty
      mempty
      mempty
...
rules = do
  ...
  let helper qg f = do
        ...
        g <- qg
        bind <- [|(>>=)|]
        pure $ InfixE (Just g) bind (Just e')
  let ur f = do
        let env =
              NP.Env
        ...
        f env
  pure $ NP.HamletRules ah ur $ \_ b -> pure $ ah `AppE` b
...
ihamletToHtml ih = do
  ...
  pure $ ih (toHtml . mrender) urender
