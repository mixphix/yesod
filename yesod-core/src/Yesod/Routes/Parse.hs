...
splitSpaces "" = []
splitSpaces str =
  let (rest, piece) = parse $ dropWhile isSpace str
   in piece : (splitSpaces rest)
 where
...
piecesFromString ::
  String -> ([(CheckOverlap, Piece String)], Maybe (CheckOverlap, String))
...
pieceFromString ::
  String -> Either (CheckOverlap, String) (CheckOverlap, Piece String)
...
lineContinuations this below@(next : rest) = case unsnoc this of
  ...
  _ -> this : below
 where
  unsnoc s = if null s then Nothing else Just (init s, last s)
