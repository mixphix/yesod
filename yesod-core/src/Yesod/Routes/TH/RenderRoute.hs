...
instanceNamesFromOpts MkRouteOpts{..} =
  prependIf roDerivedEq ''Eq $
    prependIf roDerivedShow ''Show $
      prependIf roDerivedRead ''Read []
 where
  prependIf b = if b then (:) else const id
...
-- @since 1.6.25.0
mkRouteConsOpts ::
  RouteOpts -> Cxt -> [(Type, Name)] -> [ResourceTree Type] -> ([Con], [Dec])
  ...
  go (ResourceLeaf res) = do
    let cnt =
          length (filter isDynamic $ resourcePieces res)
            + maybe 0 (const 1) (resourceMulti res)
    dyns <- replicateM cnt $ newName "dyn"
    sub <-
      case resourceDispatch res of
...
-- @since 1.6.25.0
mkRenderRouteInstanceOpts ::
  RouteOpts -> Cxt -> [(Type, Name)] -> Type -> [ResourceTree Type] -> Q [Dec]
