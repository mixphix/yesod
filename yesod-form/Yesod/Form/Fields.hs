...
import Blaze.ByteString.Builder (toLazyByteString, writeByteString)
...
import Yesod.Persist (Filter, Key, SelectOpt, selectList)
...
intField ::
  (Monad m, Integral i, RenderMessage (HandlerSite m) FormMessage) => Field m i
intField =
  Field
...
doubleField ::
  (Monad m) => (RenderMessage (HandlerSite m) FormMessage) => Field m Double
doubleField =
  Field
    { fieldParse = parseHelper $ \s ->
...
timeField ::
  (Monad m) => (RenderMessage (HandlerSite m) FormMessage) => Field m TimeOfDay
...
htmlField ::
  (Monad m) => (RenderMessage (HandlerSite m) FormMessage) => Field m Html
htmlField =
  Field
...
parseDate =
  maybe (Left MsgInvalidDay) Right
    . readMay
    . replace '/' '-'
...
parseTime =
  either
    (Left . fromMaybe MsgInvalidTimeFormat . readMay . drop 2 . dropWhile (/= ':'))
    Right
...
instance Functor OptionList where
  ...
  fmap f (OptionListGrouped options readExternal) =
    OptionListGrouped
      (map (\(g, os) -> (g, (fmap . fmap) f os)) options)
      (fmap f . readExternal)
...
mkOptionList os =
  OptionList
    ...
    , olReadExternal =
        flip Map.lookup $
          Map.fromList $
            map (optionExternalValue &&& optionInternalValue) os
    }
...
mkOptionListGrouped os =
  OptionListGrouped
    { olOptionsGrouped = os
    , olReadExternalGrouped =
        flip Map.lookup $
          Map.fromList $
...
enumerateSublists xss =
  let yss :: [(Int, (a, [b]))]
      yss =
        snd $
          foldl (\(i, res) xs -> (i + (length . snd) xs, res ++ [(i, xs)])) (1, []) xss
...
selectFieldHelper ::
  ...
  HandlerFor site (OptionList a) ->
  Field (HandlerFor site) a
selectFieldHelper outside onOpt inside grpHdr opts' =
  Field
    { fieldParse = \x _ -> do
    ...
    , fieldView = \theId name attrs val isReq -> do
        outside theId name attrs $ do
          ...
          unless isReq $
            onOpt theId name $
              render optsFlat val `notElem` map optionExternalValue optsFlat
 ...
 where
  render _ (Left x) = x
  render opts (Right a) =
    maybe "" optionExternalValue $
      listToMaybe $
        filter ((== a) . optionInternalValue) opts
...
-- @since 1.7.6
datetimeLocalField ::
  (Monad m) => (RenderMessage (HandlerSite m) FormMessage) => Field m LocalTime
datetimeLocalField =
  Field
