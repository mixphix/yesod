...
template Feed{..} render =
 ...
 where
  ...
  root =
    Element "feed" Map.empty $
      map NodeElement $
        Element "title" Map.empty [NodeContent feedTitle]
          : Element
            "link"
            (Map.fromList [("rel", "self"), ("href", render feedLinkSelf)])
            []
          ...
          : Element
            "author"
            Map.empty
            [NodeElement $ Element "name" Map.empty [NodeContent feedAuthor]]
...
entryTemplate FeedEntry{..} render =
  Element "entry" Map.empty $
    map NodeElement $
      ...
      , Element
          "content"
          (Map.singleton "type" "html")
          [NodeContent $ toStrict $ renderHtml feedEntryContent]
      ]
