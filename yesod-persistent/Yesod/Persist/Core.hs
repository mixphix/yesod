...
-- Since 1.2.0
defaultGetDBRunner ::
  (SQL.IsSqlBackend backend, YesodPersistBackend site ~ backend) =>
  (site -> Pool backend) ->
...
get404 key = do
  mres <- get key
  maybe notFound' pure mres

-- | Get the given entity by unique key, or return a 404 not found if it doesn't
--   exist.
getBy404 ::
  (PersistUniqueRead backend, PersistRecordBackend val backend, MonadIO m) =>
  Unique val ->
...
insert400_ ::
  ...
  val ->
  ReaderT backend m ()
insert400_ datum = void (insert400 datum)
