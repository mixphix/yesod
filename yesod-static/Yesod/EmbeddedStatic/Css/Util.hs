...
parseBackgroundImage n v =
  ( n
  , case P.parseOnly parseUrl v of
      Left _ -> Left v -- Can't parse url
      Right url ->
        -- maybe we should find a uri parser
...
cssProductionFilter ::
  -- | a filter to be run on production
...
cssProductionFilter prodFilter loc file =
  def
    ...
    , ebDevelReload =
        [|develPassThrough $(litE (stringL loc)) $(litE (stringL file))|]
    , ebDevelExtraFiles = Nothing
    }

cssProductionImageFilter ::
  (FilePath -> IO BL.ByteString) -> Location -> FilePath -> Entry
