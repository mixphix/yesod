...
import Network.HTTP.Types (Status (..), status200, status304, status404)
...
instance RenderRoute EmbeddedStatic where
  data Route EmbeddedStatic
    = EmbeddedResourceR [T.Text] [(T.Text, T.Text)]
...
data ComputedEntry = ComputedEntry
  ...
  -- ^ Optional haskell name to create a variable for the route
  , cStEntry :: Static.EmbeddableEntry
  -- ^ The entry to be embedded into the executable
  , cLink :: ExpQ
...
devEmbed e = pure computed
 where
  ...
  link =
    [|
      EmbeddedResourceR (T.splitOn (T.pack "/") $ T.pack $(mkStr $ ebLocation e)) []
      |]
...
prodEmbed e = do
  ...
  let hash = base64md5 ct
      link =
        [|
          EmbeddedResourceR
            (T.splitOn (T.pack "/") $ T.pack $(mkStr $ ebLocation e))
...
tryExtraDevelFiles ::
  [[T.Text] -> IO (Maybe (MimeType, BL.ByteString))] -> Application
...
develApp ::
  StaticSettings ->
  [[T.Text] -> IO (Maybe (MimeType, BL.ByteString))] ->
  Application
